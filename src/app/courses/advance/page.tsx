...
function AdvancedComposition() {
  const advancedCompositionCourse = courseData.courses.filter(
    (course: Course) => course.courseType === "advance"
  );
  return (
      ...
      <div className="max-w-7xl mx-auto px-6">
        <header className="text-center mb-12">
          <h1 className="text-3xl md:text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-pink-400 via-violet-400 to-indigo-400">
            Advanced Composition Collection
          </h1>
          <p className="mt-4 text-neutral-400 max-w-2xl mx-auto">
            {`Handpicked advanced courses (${advancedCompositionCourse.length}) for composers and producers ready to push creative boundaries — orchestration, sound design, arrangement, and production workflows.`}
          </p>
        </header>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {advancedCompositionCourse.map((course: Course) => (
              ...
              <BackgroundGradient className="w-full max-w-sm rounded-3xl p-6 backdrop-blur-md bg-white/5 border border-white/6 shadow-lg hover:shadow-violet-600/20 transition-transform duration-300 hover:-translate-y-2">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="mt-4 text-xs text-neutral-500">
                    Taught by industry professionals • Certificate included
                  </p>
                </div>
              </BackgroundGradient>
            </div>
